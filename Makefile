...
help: ## Show this help message
	@echo "GTFS Validator Go - Available commands:"
	@echo
...
build: ## Build the CLI binary
	...
	@mkdir -p $(BUILD_DIR)
	@$(GO) build -ldflags="$(LDFLAGS)" -o $(BUILD_DIR)/$(BINARY_NAME) ./cmd/gtfs-validator
	@echo "Built: $(BUILD_DIR)/$(BINARY_NAME)"

build-all: ## Build binaries for all platforms
	@echo "Building for all platforms..."
	@mkdir -p $(BUILD_DIR)
	@GOOS=linux GOARCH=amd64 $(GO) build -ldflags="$(LDFLAGS)" -o $(BUILD_DIR)/$(BINARY_NAME)-linux-amd64 ./cmd/gtfs-validator
	@GOOS=linux GOARCH=arm64 $(GO) build -ldflags="$(LDFLAGS)" -o $(BUILD_DIR)/$(BINARY_NAME)-linux-arm64 ./cmd/gtfs-validator
	@GOOS=windows GOARCH=amd64 $(GO) build -ldflags="$(LDFLAGS)" -o $(BUILD_DIR)/$(BINARY_NAME)-windows-amd64.exe ./cmd/gtfs-validator
	@GOOS=darwin GOARCH=amd64 $(GO) build -ldflags="$(LDFLAGS)" -o $(BUILD_DIR)/$(BINARY_NAME)-darwin-amd64 ./cmd/gtfs-validator
	@GOOS=darwin GOARCH=arm64 $(GO) build -ldflags="$(LDFLAGS)" -o $(BUILD_DIR)/$(BINARY_NAME)-darwin-arm64 ./cmd/gtfs-validator
	@echo "Built all platform binaries in $(BUILD_DIR)/"

install: ## Install the CLI binary
	@echo "Installing $(BINARY_NAME)..."
	@$(GO) install -ldflags="$(LDFLAGS)" ./cmd/gtfs-validator
...
docker-build: ## Build Docker image
	...
	@docker build -t gtfs-validator:$(VERSION) .
